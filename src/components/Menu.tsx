...
const Menu = () => {
  ...
  return (
    <section
      id="menu"
      className="py-16 text-center bg-gradient-to-br from-gray-50 via-white to-gray-100 font-sans relative overflow-hidden"
    >
      {/* Heading */}
      <motion.h2
        className="text-4xl md:text-6xl font-extrabold text-gray-900 mb-2 tracking-tight font-playfair"
      ...
      <motion.p
        className="text-lg text-gray-600 mb-12 italic font-light font-lato"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        ...
        <div className="flex space-x-8 px-8 sm:px-16">
          ...
          {menuCategories.map((item, index) => (
            <Link key={index} to={`/menu/${item.name}`}>
              ...
              >
                <div className="w-60 h-72 bg-white rounded-xl overflow-hidden shadow-xl mb-4 relative border-2 border-gray-200 hover:border-pink-600 transition-all duration-300 glitter-card hover:shadow-pink-500/40">
                  {/* Image */}
                  <img
                ...
                {/* Title */}
                <span className="text-lg font-semibold text-gray-800 uppercase text-center group-hover:text-pink-600 transition-colors duration-300 tracking-wide font-lato glow-text">
                  {item.name}
                </span>
              </motion.div>
            </Link>
      ...
      <AnimatePresence>
        {tooltip.visible && (
          <motion.div
            className="fixed px-3 py-1 bg-white text-pink-600 text-xs rounded-md shadow-lg border border-gray-300 pointer-events-none z-50 font-lato"
      ...
      <style>{`
        ...
        .glow-text {
          text-shadow: 0 0 6px rgba(236, 72, 153, 0.4), 0 0 10px rgba(236, 72, 153, 0.2);
        }
      `}</style>
    </section>
  );
